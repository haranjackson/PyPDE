...
/// Knyazev 	Theoretical background of the procedure can be found in the
/// articles below
...
/// sparse 	Eigen::SparseMatrix<long double> X; // N*M - initial
/// approximations to eigenvectors (random in general case)
/// Spectra::LOBPCGSolver<long double>
/// solver(A, X); 	*Eigen::SparseMatrix<long double> B; // N*N - Ax =
/// lambda*Bx, sparse, positive definite 	solver.setConstraints(B);
/// *Eigen::SparseMatrix<long double> Y; // N*K - constraints, already found
/// eigenvectors 	solver.setB(B); 	*Eigen::SparseMatrix<long double> T; //
///N*N - preconditioner ~ A^-1 	solver.setPreconditioner(T);
///
///	*** OUTCOMES
...
/// Matrix((a).triangularView<Eigen::Lower>()).transpose(); 	for (int i = 0;
/// i < 10; i++) 		a(i, i) = i + 0.5; 	std::cout << a << "\n";
